...
def top_H_T(time, rdd, H, T):
    ...
    try:
        ...
        rowRdd = rowRdd.filter(lambda p: unix_cur_time - int(datetime.datetime.strptime(p[0], '%Y-%m-%d %H:%M:%S.%f').timestamp()) <= T) \
            .map(lambda p: Row(protocol=p[1], src_ip=p[3], data_size=p[5]))
...
def top_k(time, rdd, k, T):
    ...
    try:
        ...
        unix_cur_time = int(time.timestamp())
        rowRdd = rowRdd.filter(lambda p: unix_cur_time - int(datetime.datetime.strptime(p[0], '%Y-%m-%d %H:%M:%S.%f').timestamp()) <= T) \
            .map(lambda p: Row(protocol=p[1], src_ip = p[3]))
...
def x_more_than_stddev(time, rdd, X, T):
    print("\n========= %s =========" % str(time))
    try:
        r = redis.StrictRedis(
            host='localhost',
        ...
        rowRdd = rowRdd.map(lambda p: Row(ts=datetime.datetime.strptime(p[0], '%Y-%m-%d %H:%M:%S.%f'),
                                          protocol=p[1], src_ip=p[3], data_size=p[5]))
        df = spark.createDataFrame(rowRdd)
        df.createOrReplaceTempView("services")
